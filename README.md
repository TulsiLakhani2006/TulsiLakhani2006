<div align="center">

<img src="https://raw.githubusercontent.com/ABSphreak/ABSphreak/master/gifs/Hi.gif" width="50px" />

<img src="https://readme-typing-svg.herokuapp.com?font=Orbitron&weight=600&size=50&duration=3000&color=7CF5FF&center=true&vCenter=true&width=550&lines=Tulsi+Lakhani" />

<img src="https://readme-typing-svg.herokuapp.com?size=22&duration=3000&color=FFFFFF&center=true&vCenter=true&width=700&lines=IT+Student+%7C+Developer+%7C+Problem+Solver;Always+Learning+New+Things+✨" />

<br/>

### ✨ *“Building simple, useful and meaningful software.”* ✨
</div>

---

## 💡 Core Developer Profile 🚀

> 💫 *Focus: Clean Code, Strong Logic, and Real-World Application.*

| **Philosophy** | **Focus** | **Goal** |
| :--- | :--- | :--- |
| **Clean & Minimal Code** 🧹 | **High-Quality Projects** (Quality > Quantity) 💎 | **Aspiring Full-Stack Engineer** 💻 |
| **Strong DSA Base** 🧠 | Solving **Real-World Problems** 🌍 | **Dedicated to Continuous Learning** ✨ |

---

## 🎓 About Me 🧑‍💻

<div align="center">
    <img src="https://capsule-render.vercel.app/api?type=waving&color=7CF5FF&height=50&section=header"/>
</div>

* **3rd-year IT student.** 📚
* **Loves designing clean UI/UX.** 🎨
* **Builds real-world projects.** 🛠️
* **Practices DSA daily (LC/GFG).** 📈
* **Learning Full-Stack & System Design.** 🌐

---

## 📚 Focused Growth 🌱

<details>
<summary> **Current Strategic Learning Roadmap** </summary>
<br>

* **Frontend:** Advanced React.js concepts (Performance, Hooks, Context). ⚛️
* **Backend:** Node.js/Express.js for robust API development. ⚙️
* **Data:** SQL optimization and efficient database management. 🗄️
* **Algorithmics:** Mastering complex DSA patterns (DP, Sliding Window). 📊
</details>

---


## 🔧 Technical Toolkit 🧰

### **The Technology Stack (Comprehensive View)**

| **Category** | **Skills / Tools** |
| :--- | :--- |
| **Languages** | `C` • `C++` • `Java` • `Python` • `JavaScript` • `SQL` |
| **Web (Frameworks/Core)** | `HTML` • `CSS` • `React.js` |
| **Databases** | `MySQL` • `PostgreSQL` |
| **Tools / Platforms** | `Git` • `GitHub` • `Visual Studio Code` • `NetBeans` |
| **Achievements** | `LeetCode` • `GeeksforGeeks` • `CodeChef` |

<br>

### **Badges for Visual Impact**

<div align="center">

<img src="https://img.shields.io/badge/C-A8B9CC?style=for-the-badge&logo=c&logoColor=white" />
<img src="https://img.shields.io/badge/C%2B%2B-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white" />
<img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=java&logoColor=white" />
<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
<img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black" />
<img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" />
<img src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white" />
<img src="https://img.shields.io/badge/Visual_Studio_Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white" />
<img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />

</div>

---

## 🖥️ My Development Environment 💻

> Utilizing **VS Code** and **NetBeans** for efficient development, with **Git/GitHub** for robust version control.

<br>
<img src="https://capsule-render.vercel.app/api?type=waving&color=7CF5FF&height=100&section=footer"/>
